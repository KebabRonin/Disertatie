"""Simple examples of using the "frams" module to communicate directly with the Framsticks library (dll/so/dylib).
...
import sys
import json
import numpy as np  # only needed for the last part of this script (sampling a Model in 3D). Must be imported before frams.init(), because that init() call unmasks FPU exceptions like division by 0.0. Since numpy on initialization purposefully triggers a float division by zero, it needs masked exceptions or else its initialization will silently kill this process (on Windows with exit code 0xC000008E = EXCEPTION_FLT_DIVIDE_BY_ZERO).
import frams
...
for p in m.voxels():
	...
	matrix[int(p.x._value() * 7 + 1), int(p.y._value() * 7 + 8), int(p.z._value() * 7 + 6)] += 1  # scaling and offsets adjusted manually to fit the matrix nicely
matrix = np.sum(matrix, axis=1)  # sum along axis, make 2D from 3D ("projection")
gradient = "oO░░▒▒▓▓██"  # if you prefer a "visual gradient" instead of numbers 0-9, use gradient[x//8] below
np.set_printoptions(formatter={'int': lambda x: ('.' if x == 0 else str(x // 8))})  # print zeros as dots, x//8 to fit a larger range into a single digit 0-9
print(matrix)
