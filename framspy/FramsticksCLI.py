...
class FramsticksCLI:
	...
	def __init__(self, framspath, framsexe, pid=""):
		...
		try:
			...
			output = self.__readAllOutput(exe_call_to_get_path)
			for line in output:
				if 'dDIRECTORY' in line:
					self.writing_path = line.split("'")[1]
	...
	def sendDirectCommand(self, command: str) -> str:
		...
		STDOUT_ENDOPER_MARKER = "unique-marker-" + str(FramsticksCLI.sendDirectCommand_counter)
	...
	def dissimilarity(self, genotype_list: List[str]) -> np.ndarray:
		...
		# This would be too complicated, so strings (names) in column #1 become NaN as floats (unless they happen to be valid numbers) - not great, not terrible
